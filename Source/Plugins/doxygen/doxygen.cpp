...
void Doxygen::createDocumentation(const DoxygenSettingsStruct &DoxySettings)
{
    ...
    TextEditor::TextEditorWidget *editorWidget = qobject_cast<TextEditor::TextEditorWidget*>(
                Core::EditorManager::instance()->currentEditor()->widget());
    ...
    if(lastSymbol->isClass())
    {
        ...
        if(DoxySettings.verbosePrinting)
        {
            ...
            QString filePath = editor->document()->filePath().toString(); //ERROR: CHECK IT
            QString fileName = filePath.remove(0, filePath.lastIndexOf("/") + 1);
            QString fileNameProj = filePath.remove(projectRoot);
...
void Doxygen::documentProject(ProjectExplorer::Project *p, const DoxygenSettingsStruct &DoxySettings)
{
    ...
    for(int i = 0 ; i < files.size() ; ++i)
    {
        ...
        if(
            ...
            Core::IEditor *editor = Core::EditorManager::instance()->openEditor(files[i]);
            if(editor)
            {
                documented = true;
