...
const QCommandLineConfigEntry QCommandLine::helpEntry = { QCommandLine::Switch, QLatin1Char('h'), QLatin1String("help"), QObject::tr("Display this help and exit"), QCommandLine::Optional };

const QCommandLineConfigEntry QCommandLine::versionEntry = { QCommandLine::Switch, QLatin1Char('V'), QLatin1String("version"),  QObject::tr("Display version and exit"), QCommandLine::Optional };
...
QCommandLine::parse()
{
  ...
  foreach (const QCommandLineConfigEntry& entry, d->config) {
  ...
  foreach (const QString& key, switchs) {
  ...
  foreach (const QString& key, options) {
    foreach (const QString& opt, optionsFound[key])
      emit optionFound(key, opt);
  }

  foreach (const QCommandLineConfigEntry& entry, params) {
    if (entry.flags & QCommandLine::Mandatory) {
  ...
  QList<QCommandLineConfigEntry> valuesOf = conf.values();
  for (int counter=0; counter < valuesOf.count(); counter++) {
     const QCommandLineConfigEntry& entry = valuesOf.at(counter);
    if (entry.flags & QCommandLine::Mandatory) {
      QString type;
...
QCommandLine::help(bool logo)
{
  ...
  {
      h += QLatin1Char('[') + d->description + QLatin1String("]:") ;
  }
  h += QLatin1Char('\n');
  /* Executable name */
  if (!d->args.isEmpty())
